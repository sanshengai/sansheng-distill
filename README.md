...
心理学书可以启用严格域,把「原书怎么说」和「外部研究怎么说」拆成两层:每条主张都有稳定 `claim_id`,页面并列显示原书主张、科学证据、适用边界与风险。`source-audit.json` 再把 `book.txt`、`distill.json`、固定同目录的 `claim-coverage.json` 与 `enrich.json` 锁到同一组 SHA-256,逐项记录原文行号。严格验收必须显式带原文:
...
这是一组**条件契约**:普通书与旧页面不启用时行为不变。机器闸能抓输入错配、漏覆盖和结构假绿,但不能替代对研究语境与关键反例的人工复审。

跨书还有两条确定性聚合路径:同一作者 ≥2 部可生成思想演变页,同一主题 ≥3 本可生成分类、真分歧、平行对照与维度比较页。显式成员清单缺一项就拒绝生成,不会静默缩小集合;自定义书页地址只接受安全的站内根相对路径。真分歧与「回答不同问题的互补镜头」分别进入分歧卡和 `parallel_comparisons` 平行对照卡,避免把并列观点包装成冲突。
