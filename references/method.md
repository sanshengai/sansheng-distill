...
> 本节记录叙事重述试点；其字数比例不能代替独立知识覆盖。**要求保留单本管理书大部分知识时，优先走 [high-retention-books.md](high-retention-books.md)**：先冻结知识分母，全书 ≥90%、每有效原章 ≥80%、核心观点及改变结论的条件/限制在盘点时标记 `critical` 并完整覆盖 100%、有效源段去向 100%，并核图意、语义审阅与三层页面。公开 Skill 自持此通用契约，不依赖另一个私有产品 Skill 的安装路径。叙事试点的等密比 40–80% 与源段覆盖 ≥70% 仅适用于该试点口径，不可用来宣称管理知识档达标。

### 3.7.1 单位与目标

- **按原书的节切**(目录里的「1.小节」/ 访谈的话题标题),3–5 千字一节;不按 6,500 字硬切,节是读者的阅读单位。**切之前先人肉看目录页和正文标题的写法**——七本书碰到四种排版,`diagnose.json` 的 `chapters_detected` 对节级切分没用:
  | 排版 | 例 | `split --mode` |
  |---|---|---|
  | 有「目录」行,「一、大章」+「1.小节」 | 店长日记 | `toc-cn` |
  | 书开头一整块目录清单,无「目录」二字 | 你学不会 | `toc-list`(顶部连续短行即清单,正文里与清单逐字相同的行才是标题) |
  | 「第X章」下只有短行小标题,或「第X节」 | 以慢制胜、凭什么、店长日记 2 | `chapter-subhead [--sec-regex '^第[一二三四五六]+节']` |
  | epub 把标题竖排成单字行、访谈带「李翔：/王宁：」 | 因为独特、何以泡泡玛特 | `vertical [--part-regex '^第\\d+章' --sec-regex '^\\d+\\.\\d+']` |
  切完核对:节数 × 平均字数 ≈ 目录;目录出现两次(目录页 + 正文)只认正文那份。短节先检查实质内容，再决定独立保留或并入同章邻节，不能仅因 <300 字自动丢弃；版权页、他人推荐序等需登记排除理由，自序、实质脚注和附录保留。
- **体裁定模板**(`units.json` 的 `kind`,`split --kind` 给整本默认,混合书按章改):`narrative`(日记 / 传记 / 故事,第三人称讲人)、`interview`(问答改叙述)、`essay`(论说型:顺着作者的论证讲,案例整段讲完,「作者认为」)。《你学不会》第四、六章是分析,其余是故事,一本书里两种。
...
**两轮为止**:七本实测第二轮收益已小、偶有反弹(何以泡泡玛特 29 → 24),第三轮不做;残余按类看一眼——日期序列、机构与公司名清单、口号座右铭豁免(以慢制胜剩 47 处全是这类)。模型为躲 JSON 转义会用英文单引号和全角破折号,`collect` 统一规整成「」和 `--`。
...
- **旁证池要有同题的第二本书才值得开**:海底捞四本互为旁证,每本插 23–33 处,同一件事(杨小丽、袁华强、A 级店)在四本里各有角度;《何以泡泡玛特》只有一本同题书加两篇报道,全书只插了 1 处。单本书不必配旁证。
...
- **人物 / 关系 / 因果对账**:书层已有 `scripts/check_deep_retell.py prepare` 生成语义抽检计划、`collect` 收集审阅、`check-report` 核报告；应先查当次报告是否实际完成，不能以工具存在代替审阅。每本抽 10% 小节逐段对原文，高风险主体、日期与否定优先；错误超过被抽检断言的 5% 时扩到该错误类型的全书检查。该工具服务叙事重述抽检，不能替代管理知识档的全量知识盘点与逐项来源—目标正文审阅。
- **速览层检验是交付物**:`deep_retell.py glance` 导出 `deepread-glance.md`,按一页宪法 §八 单看标题能否讲出这本书;标题必须是事件句,不要俏皮话。
...
- `deep_retell.py render` 出 `deepread-pilot.html`:三层折叠(节 → 小节 → 段),工具条「全部展开 / 收起」,收起态 = 节名 + 关键词(日期排最后),直引挂在与它最像的那一段后面,页脚出处声明。是否并入 v3 五 tab 页型(作为「深读」tab 或独立页)待 sandy 看过两本试点再定,**未定前不动 `RENDER_PROFILES`**。
- 落盘:`$DATA/<书目录>/deepread.json` 与 `deepread-pilot.html` 为公开表达候选；`$DATA` 沿用数据目录约定。含全文的切片、任务和模型回执放私有工作目录，实际验证其被版本控制与网站投影排除。高保留知识档的 `deepread.json` 字段和三层页面另见 [high-retention-books.md](high-retention-books.md)，不与本试点 schema 混用。
