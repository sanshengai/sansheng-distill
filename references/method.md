...
## 3.7 深读重述层(deep retell,2026-09-22 两本试点定)

> **为什么有这一层**:讲书稿(§3.5,800–1500 字/章)是给「愿读 30 分钟的人」;sandy 2026-09-22 复盘企业沉浮六家后要的是另一件事——**书里六到七成的内容留在网页上,读起来像书**(故事从 1 到 2 到 3、文字流畅),浅读靠折叠看节名和关键词,深读点开看细部。§3.5 的漏斗留不下这么多(七本书 79.6 万字进企业故事页只剩 7.4%),所以另立一层,不改 §3.5。试点两本:《海底捞店长日记》(日记体,29 节)、《因为独特》(访谈体 + 口述,72 节),脚本 `scripts/deep_retell.py`。

> 术语与阈值对齐 sansheng-yiye `deep-prose.md`（一页内容层宪法）：**等密重写**（等密比 40–80%）、**源段覆盖 ≥70%**（反向覆盖率闸门，静默跳过不允许）、**双层结构**（速览层 = 节名 + 小节标题 + 关键词，详述层 = 正文）。本节只写书层的具体做法，阈值以宪法为准。

### 3.7.1 单位与目标

- **按原书的节切**(目录里的「1.小节」/ 访谈的话题标题),3–5 千字一节;不按 6,500 字硬切,节是读者的阅读单位。目录识别失败(竖排单字标题、访谈说话人标签)用 `split --mode vertical`。
- **每节保留六到七成内容**:发生了什么、何时何地、谁、做了什么、说了什么、数字、作者当时的想法;只删重复、过渡、客套。实测长度比 0.74–0.75(两本合计 249,500 → 186,096 字)。
- **长度比不是覆盖率**:《店长日记》「真正的智慧」一节长度比 0.49,逐句对原文却是全覆盖——原书大段引克里希那穆提和许巍歌词,重述只提不抄(第三方作品不重述)。所以长度比只做报告,覆盖靠 §3.7.5 抽检。

### 3.7.2 写法(提示词硬规,`deep_retell.py` TPL_NARR / TPL_INTV)

- 像讲故事一样连贯:按事件顺序推进,段与段有时间和因果衔接;**禁要点罗列、禁「首先/其次」、禁总结句、禁「本节告诉我们」**。
- 第三人称;作者自己的判断写成「他认为」;不添加原文没有的事实、人物、数字,原文没写的心理活动不猜。
- **每节 2–5 个小节**,每小节一件事:8–16 字事件式小标题(说发生了什么,禁「之道 / 的力量 / 启示」)+ 3–5 个关键词(人 / 地 / 物 / 数字)。收起态只显示节名 + 关键词(日期排最后),这就是浅读层。
- 访谈体:提问只当引子(「被问到……时,他说……」),不逐条搬问答;口述体:「某某回忆说」。每章末的「专家点评」压成一段,放最后一个小节。
- 破折号 `--`;不出现「本章 / 本节 / 原文 / 访谈 / 蒸馏」等元话语。

### 3.7.3 原话与改写线(机检,`collect` 报告)

| 项 | 判据 | 处置 |
|---|---|---|
| 原话 | 每小节 ≤1 句,逐字,≤150 字,只在非原话不可时用 | 不逐字的一律置空(宁可少一句,不冒充原话);全书逐字合计仍守 §3.5.4 的 ≤10% |
| 连续雷同 | 引号外**连续 30 字与原文相同**即打回 | `rewrite` 整节重派,附上被点名片段;两轮后残余只剩专名清单、口号、座右铭(豁免,不再追) |
| 贴近度 | 20 字窗口命中率(报告项) | glm-5.3 首稿约 4%;>10% 说明在抄不是在讲 |
| 数字 | 输出里的数字必须出现在本节原文或旁证里(书名括号里的年份不算) | 无据数字随 `rewrite` 一起改 |
| 源段覆盖 | 原文逐段编号 [P12];每小节声明 `covers`,没写的段必须进 `skipped` 并给原因(他人作品引文 / 纯过渡客套 / 与前文重复);声明再过三字组弱重合核实(<0.12 = 假声明) | 覆盖 <70% 或有未交代段 → `rewrite` 点名补写;跳过理由进 deepread.json 可查 |

实测:glm-5.3 首稿 30 字雷同 97 处 / 29 节(店长日记)、149 处 / 72 节(因为独特);一轮重写后 15 / 9,二轮后 6 / 9。JSON 坏 6 处全是字符串里的英文双引号,`loads` 先本地成对换「」再报坏。

### 3.7.4 旁证叠加(sandy:「每本书都有同样的故事,只是角度不一样,可以叠加」)

- 旁证只取**叙事型**来源(书 / 长报道 / 访谈 / 口述 / 案例;年报、招股书不算——「另一本书《…年报》」很怪)。从企业研究包的 `claims.json` 按三字组重合挑 top 6 给写手,提示词限「确实是同一件事才用,最多 2 处,写明书名 / 报道名」。
- 写进正文的形式:「《海底捞你学不会》里也记着……」「据《晚点》的报道……」。书名括号里的作者年份在 `collect` 时剥掉。

### 3.7.5 模型与抽检

- **起稿 glm-5.3(方舟 Agent Plan)**,8 路并发;29 节 + 72 节两轮改写合计约 25 分钟,不占 Opus 额度。Opus 同题样节:长度比 0.89–1.0、雷同 0、文字更细腻(会把访谈里散开的一条线收拢),但单节 8.4 万 token——101 节要 850 万,只做抽检和坏节重写。
- **抽检**:每本至少 3 节逐段对原文(比例最低的、雷同最多的、随机一节),看漏掉的是什么(第三方文本 / 过渡 = 正常;事件 / 数字 / 转折 = 打回)。

### 3.7.6 版权(与 sandy 2026-09-22 的讨论结论)

- 事实、事件、人物、数字、时间线**不受著作权保护**,六到七成内容靠改写留下来,这一点没有法律障碍;§3.5.4 的 150 字 / 10% 是**逐字**的线,不是内容的线,两者别混。
- 改写不等于零风险:法律保护的是表达,把一本书按原顺序逐句改写成「不用买书的替代品」,司法实践里可能被认定实质性相似 / 改编。降风险的做法正是 sandy 自己提的:按事件重排结构、多本书 / 报道叠加、不逐字、页脚声明「本页是重述不是原文,要读原文请买原书」。**原书里引用的第三方作品(他人书摘、歌词、诗)只提及不重述。**
- 这条线由 sandy 拍板(她要打破的是我们自设的 150 / 400 / 5% 之类内容比例,不是逐字线);以后按书单独判断时记到该书目录 `deepread.json` 顶层 `copyright_note`。

### 3.7.7 页面与落盘

- `deep_retell.py render` 出 `deepread-pilot.html`:三层折叠(节 → 小节 → 段),工具条「全部展开 / 收起」,收起态 = 节名 + 关键词,页脚出处声明。是否并入 v3 五 tab 页型(作为「深读」tab 或独立页)待 sandy 看过两本试点再定,**未定前不动 `RENDER_PROFILES`**。
- 落盘:`读书蒸馏/<书目录>/deepread.json`(产物,进 Git)、`deepread-pilot.html`(进 Git);`_deepread/`(切片、任务、方舟产物,含全文)在 `.gitignore`。
...
> **v6.3 严格心理学原文审计(并入 G24)**:项目调用 `--require-domain psychology` 时,同书目录必须有 `source-audit.json`(`psychology-source-audit-v1`,完整契约见 `source-audit.md`)。验证器复算 source/distill/claim-map/enrich 四 hash,核 source 行数、连续章界与章头、records 唯一/目标存在/章内范围/≤150 字逐字命中；最终 claims、每章 narrative 多 facet、quotes/excerpts 须有来源记录，全部 claim-map audit flags 须一一入账，evidence 须精确覆盖最终 claims。该门只在显式严格域激活,默认路径与旧书不变。
