...
- **按原书的节切**(目录里的「1.小节」/ 访谈的话题标题),3–5 千字一节;不按 6,500 字硬切,节是读者的阅读单位。**切之前先人肉看目录页和正文标题的写法**——七本书碰到四种排版,`diagnose.json` 的 `chapters_detected` 对节级切分没用:
  ...
  切完核对:节数 × 平均字数 ≈ 目录;目录出现两次(目录页 + 正文)只认正文那份;<300 字的节丢、<600 字并入同章前一节;参考资料、版权页、他人推荐序不进单元,自序保留。
...
- 🔴 **还欠一道强模型对账**(2026-09-22 七本时未做,列为待补):企业沉浮的 J3(glm-5.3 / Opus 查主语错位、亲属关系、日期)在书层没有对应物,机检只看数字和雷同,抓不到「张红超摔倒」写成「张红甫」这类主语错位。补法:每本抽 10% 小节派强模型逐段对原文,报「人物 / 关系 / 因果」三类错;错误率 >5% 整本重跑该体裁模板。
