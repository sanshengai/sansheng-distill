...
## 5.3 同作者跨书去重(v6,2026-07-12 B-6)

**同一作者多本书共用同一母题 / 公式时,别在每本书页各自全文复述。** 2026-07-12 复盘:稻盛《活法》《干法》《心》共用同一「结果=思维方式×热情×能力」公式、核心观点 70% 重叠,四书各出全 5-tab 独立页 = 三张近重复页。规则:

- **母题只在作者演变页展开一次**:同作者重复母题的深度展开归 `author.html`(StepA / `author-craft.md`),单书页对该母题**只留一句本书视角 + 互链到演变页**,不重抄整段。
- **蒸同作者第 N 本(N≥2)时先比对已蒸书**:读该作者已蒸各书的 `distill.json.core_ideas` / `napkin.formula`,**高度重叠(同一公式/同一主张)的,本书页标注「与《X》同源,详见作者演变页」并互链**,而非再写一遍。判重靠语义比对(非机检),重叠即互链去重。
- **与 §5 概念互链的区别**:§5 是「不同书的**同一概念**」横向并置(含分歧);本节是「**同一作者**跨书的**重复母题**」纵向去重(避免自我复读)。两者都靠 `knowledge-index.json` 的 book_slug 关联,渲染分工:§5 → M11 跨书观点;本节 → 作者演变入口卡 + 单书页母题处的「详见演变页」互链。
