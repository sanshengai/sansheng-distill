# Changelog

本项目的变更记录。版本号遵循 [semver](https://semver.org/lang/zh-CN/)。

## [未发布]

### 书籍深读与来源预检

- 单书深读新增连续阅读验收：沿情境、推理、选择、结果与边界组织章节；保留独有故事与论证，跨书重复只在综合层合并，不把单书拆成孤立知识卡。
- 新增 PDF 来源预检命令，提示印刷页序跳跃、近重复扫描页和低文本页；异常需回原页裁决，未报异常不代替人工查源。
- 来源缺口设止损与批次独立结算：缺正文时暂停该书的正式分母与放行，已完成的其他书继续推进；候选稿、本地页和公网实测分别报告。
- 明确引文字数检查只是内部预警；公开作品还需检查整体表达、编排与独立评论，不能把换词或拆段当作许可。

### 深读重述 collect：两处静默问题

- 改写轮（`-rw` / `-rw2`）回答 JSON 坏掉时仍退回上一轮，但现在写进 `collect-diagnostics.json` 的 `warnings`（`rewrite_response_invalid`，含退回到哪一轮），汇总行报数。原来零痕迹，「改写没效果」和「改写结果没被用上」分不清。
- 无据数字比对前两边并掉千分位空格（「7 000」=「7000」），只并「数字 + 空格 + 恰好三位数字」。原来原书排版用千分位空格时，重述写规范数字会被整批误判无据。

## [0.9.0] -- 2026-09-01

### 人物传记证据闭包

- `canonical_resolution` 新增可签署的 `source_only` 裁决：已接纳的 Observation 在没有语义同一且经审核的 Canonical 对象时，可以显式保留为“仅来源所见”；其 `target_ids` 和 `superseded_ids` 必须为空，且不得反向出现在 Canonical `evidence_refs` 中。其他 resolution 仍强制非空 target，不放宽 exact-once 与证据反链门禁。

### 修复：套装电子书入书四类静默失败

用真实的曾国藩六本电子书（12 册套装 azw3、曾文正公 16 册、家书三册、唐浩明三册、张宏杰传、智慧精髓三册）体检 `convert_book.py`，发现四个**都不报错**的失败模式。它们的共同点是 `exit 0` + 零警告，坏数据直接流进后续蒸馏与证据链。

- **切分册会切出别的书。** 有些套装的目录节点**自身也带链接**，而那个链接指向的是**别册**的正文文件。旧逻辑把它和子项链接混在一起取最小值，于是锚点落到别的书上：向 12 册套装要第 11 册《家书》，拿回来的是奏稿卷十四（31,046 字，「禀父母」零命中），诊断照样是「直接蒸馏」。现在起点与终点都只认子项链接，节点自身链接仅在没有子项时才作为回退。修后第 11 册为 390,963 字、「禀父母」47 处。每节点只有一个链接的套装（曾文正公 16 册、家书上中下）结果不变。
- **同名分册无法寻址，且会静默拿到封面页。** 唐浩明三册套装的顶层有三个节点都叫「目录」，按名字切永远只拿得到第 1 册，第 2、3 册在命令行下**根本取不到**；改按书名切则会拿到 589 字的封面加版权页，仍判「直接蒸馏」。现在：新增 `--volume-index <下标>` 按 `--list-volumes` 的数组下标寻址（该输出同时新增 `index` 字段）；`--volume` 命中多个同名节点时**报错退出并列出候选下标**，不再静默取第一个；容器文件大于 1MB 却只切出不足 5000 字时降级为「需人工确认」并退出 3。
- **中文古籍与部分中文传记的章节召回为零。** 章节头正则缺「卷」字、行首不允许装饰符、序号只认半角标点。实测曾文正公全集第一册 19 处「奏稿 卷一」命中 0；张宏杰《曾国藩传》的「｜第一章｜」（全角竖线）与「1．」（全角句点）也全部命中 0。现已覆盖：`卷`/`辑` 章名、行首可选装饰符 `｜|〔【`、全角句点序号，以及「体裁名 + 卷N」「卷之N」版式。修后两书分别召回 19 与 166 处。
- **生僻字造字图被静默丢弃。** 古籍电子书用图片表示生僻字（曾文正公 16 册正文内联图 2,975 处、家书 2,137 处），旧逻辑取文本时直接丢掉，字凭空消失而乱码率检测不到（12 册实测替换符比率仅 0.0002）。现在这些图片会转成可见标记 `〔图字:…〕`，诊断新增 `inline_glyph_images` 计数。**注意**：正文因此比旧版略长（造字图多的分册约 +0.2%~1.4%），这是补回来的信息，不是重复内容。

已知未修：行首「一部/六部」这类词仍会被当成「第N部」误报（旧有行为，不影响蒸馏）；按下标切出的分册标题取自目录节点原名，遇到「目录」这种节点名需要人工改写。

### 修复

- 修复 v2 媒体治理账本的主键识别：审计器现在按账本类型选择 `asset_id` 等正式主键，不再把媒体行中的 `changeset_id` 外键误判为跨账本重复 ID；新增回归测试覆盖合法媒体与其变更回执共存的场景。

## [0.8.1] -- 2026-08-23

### 修复

- 收紧产品侧 v1 兼容例外：显式 allowlist 现在只放行已审阅的 legacy migration 代码；任何新出现的 error、fatal 或未登记迁移规则都会 fail closed，避免旧人物迁移期掩盖新的语义损坏。

## [0.8.0] -- 2026-08-23

### biography_corpus：新增可迁移、可审计的证据型人物传记路径

这一版把「人物思想蒸馏」与「历史人物事实建库」正式拆成两条路线。用户现在可以从零初始化一个独立人物 store，把来源、逐条观察、正式裁决、六类规范事实、编辑叙事和外部核验连成可回查的证据链，再把确定性投影交给自己的产品。

- 新增实验性 v2 传记候选契约 `0.9.0-candidate`：manifest 显式声明人物身份、文件映射、治理账本、投影与发布状态；JSON Schema 和跨文件语义审计共同检查证据闭包、exact-once 裁决、redirect、关系、争议、引语、历史日期与作品版本。仓库 v0.8.0 与数据契约使用独立版本域，候选契约在稳定前仍可能调整。
- 新增单一 `biography_store.py` 入口，提供 `init`、`audit`、`verify` 与 `audit-corpus`；编译、导出和测试复用同一审计谓词，不再各写一套近似门禁。
- 外部模型（包括 GLM-5.3）只能提交 `recommendation_only`，正式裁决须由 manifest allowlist 中的 `human` 或 `main_agent` reviewer 签署。
- 每个人物的 slug、namespace、路由、资源目录和 CSS scope 独立；跨人物共享资源必须显式只读并绑定摘要，corpus 审计会拦截串线和 ID 冲突。
- 风险正文、正式变更、模型建议与媒体元数据都绑定可重算的内容摘要；本地引用拒绝绝对路径、穿越、反斜杠、ADS、控制字符与 Windows 设备名，避免“账本在、内容已换”和跨目录读取。
- 来源见证、历史日期换算、关系端点角色、引语声称者/实际说话者/译文链以及媒体真实性均进入跨文件语义审计；公开模式拒绝未核真实性或权利未清的媒体。
- 书籍、视频与既有 `creator_corpus` 路径保持兼容；人物传记契约只负责资料与验证，不绑定任何特定网站的页面、SEO 或部署实现。

## [0.7.0] -- 2026-08-21

### 心理学：新增原书主张与外部科学证据双层契约

心理学著作常把理论、相关关系、因果主张和行动建议写在同一条叙事链里。原有 `evidence_level` 只能回答「蒸馏是否忠于原书」，不能回答「书中的主张后来是否被复制、修正或推翻」。本轮在不改变既有书页默认行为的前提下，增加一层可选、可机械验收的科学证据复核。

- 心理学书可声明 `domain_profile`;核心观点与决策规则必须带稳定的 `claim_id` 和八类 `claim_type`,由 G23 检查完整性与唯一性。
- `enrich.json` 对心理学书增加条件第六键 `evidence_page`,逐条记录结论状态、最佳证据、信心、复制状态、适用范围、风险和一手来源;G24 要求与原书主张一一覆盖。
- HTML 为每条主张固定渲染「原书怎么说 / 外部研究怎么说 / 适用边界与风险」三栏,避免把作者叙述与当前科学共识混成一个“证据等级”。
- 新增整书 `source-audit.json` 原文审计账本,把原文、最终蒸馏、同目录固定 `claim-coverage.json` 和外部增补的哈希锁在同一快照中;心理学严格验证必须显式传 `--source`,章节叙事、引文、数字、边界与审计旗标均须回到明确行号,旧响应、旁路 claim map 或错书响应不能冒充当前审计。
- 主题聚合支持一本书归入多个流派;缺失或非法的转述确定性降级为 `unverified`,不再静默冒充原书明说;分歧可附问题类型、裁决与来源。
- 作者与主题聚合新增显式成员、合著限定、人工母题、书页路由和平行对照契约;显式成员缺失、损坏或 slug 错配时停止生成,`web_url` 只放行安全的站内根相对路径;真正互相排斥的主张继续进入分歧卡,只是研究问题不同的书进入独立 `parallel_comparisons` schema 与 `.cmp-card`,验证器同时核 JSON 契约和精确 DOM 数量。
- 所有新增字段均为条件契约,旧书、旧专题及非心理学产物保持兼容;对应合并、构建、验证和变异测试已补齐。

### creator_corpus（StepC）：新增 P6.5 深读层 —— 这类页面此前交付的是索引，不是读物

Hormozi 页上线后复核发现的架构级缺口，Dan Koe 那一版同病且更薄：**密度下限只规定了「有多少个条目」，从没规定「每个条目有多深」**。管线是单向收敛的——2,726 条证据 → 1,596 个族 → 11 层抽象结论，收敛途中案例、数字、推演全部丢弃，页面只呈现最后那层抽象。

量化：页面正文 20,507 字，摊到 73 个条目上每个只有 67–298 字；而后台压着 13 万字族正文、2,726 条逐字核实的原话、443 张来源卡摘要，**用掉不到 16%**。更要命的是证据回查路径是断的：层里写「证据族 11 组 · 一手来源 8 个」只有数字，点进来源库只有标题摘要，那 2,726 条具体阐述读者一个字都看不到。

**新增能力**

- **§5.5 深读层（P6.5）**：给每个结构条目配一篇长文，固定五段——他到底在论证什么 / 他拿什么撑 / 他的原话 / 什么时候不成立 / 这些年改了什么。第五段最值钱：单看某一年会得到三个不同的他，铺开九年才看得出是同一件事的三次迭代，而这是他自己从没写过的（碎片化输出的必然）。
- **深度下限表**（系统层 2,000 字 / 主题 2,500 / 模型 1,200 / 张力 1,200 / 阶段 1,500 / 桥接谱系 700），与密度下限一起才算达标。
- **料包生成规范**：七类条目各自的关联路径（主题走 `theme_id` 反查、张力走 `evidence_a/b_ids`、谱系要按检索词补料）。**层的料包必须补进同主题的高价值族**——层只挂 7–10 个而主题动辄上百，不补就够不着本层最有说服力的材料，补后料厚涨 2.5 倍。
- **§8 深读分页架构**：12 万字全塞主页会到 600KB，按 tab 拆四个子页；读者操作步骤与原来点 tab 完全一样。附一条：主页绝不能 import 那四个数据包，另出轻量 `deepread-index.json`。
- 主题类此前**从未上过主页面**（只在来源库当筛选器），拆页后成为净新增内容。

**门禁**

- **六道机械闸门**（字数 / 五段结构 / 引文逐字回查 / 数字回源 / 占位符 / AI 腔），导出前必过，判红不进数据包。深读是全管线唯一「模型自由发挥」的环节，编个数字造句引文在页面上完全看不出来。
- **第七道：中文标点归一**。实测 71 篇里 33 篇**全篇半角标点**、其余全篇全角——同一批 agent 的输出习惯会分裂成两派，共 3,141 处。半角逗号在中文里没有字间距，页面上挤成一团，而它不影响任何数据校验，所有既有闸门都零症状。
- 闸门必须做**变异测试**（注入假数字、假引文、占位符、AI 腔，四项都要报红）。
- 验收清单补三项：深度下限、闸门变异测试、**深读真的渲染到页面上**（拿构建产物比对，不是看 JSON）。

**踩坑**

- **闸门自己会误杀，且症状酷似「模型造假」**。引文回查若「先提取料包里的引号内容成列表再比对」，会因来源标题自带引号导致正则错位截断——实测 5 条真实存在的引文全被判成编造。正确做法是整包归一化后搜子串。
- **数字回查必须分级**：带单位的（`37%` / `$4000`）判红；纯整数多为合理推算（实测正文写「前六年加起来 53」，料包年份分布相加正好 53），一刀切会逼着把有价值的横向对比全删掉。
- **原话不许「修好」**：料包里的引语是转写片段，本来就可能不完整、无标点、首字母不大写。有稿子因为给引文补了一个 `The` 被判红。
- **🔴 渲染层会静默改写逐字引用**，两处：① Astro markdown 默认开 smartypants，把 `you're` 转成 `you're`——321 条原话里 80 条因此在页面上不再逐字；② 原话里的 `*` 会被当成斜体标记吃掉（`sh*t...sh*t` 整段变斜体、星号消失），导出时必须转义 `* _ [ ] \``。两者都是「内容看着完全正常、所有既有测试全绿」的静默失效，只有拿构建产物逐条比对源文才抓得到。
- **谱系的检索词不能用 `[中文]{2,4}` 从标题直接切**：中文没有词边界，「一致性原则 → CTA 连环」会被切成「一致性原」+「连环」这种废词，一个族都捞不到（实测一份料包因此是空的）。改用 2-4 字滑动窗口穷举 + 英文 token 单列，再靠「命中词数」排序压噪音——修完谱系料包从均 1,031 字符涨到 4,998。
- **模型档位**：Sonnet 足够（料包已把找料与交叉验证做完），但必须先跑 2 篇验质量再全量，且闸门兜底。实测它还会自己从证据分布里读出编辑判断——指出某条张力两边证据厚度不对称（一边跨 8 个来源两年多反复出现，另一边 5 条里 3 条同源于一支视频），因此不该被当作同等分量的定论。

### creator_corpus（StepC）：双轨 + 桥接层，以及第二个人暴露出来的一批坑

第一次给「人物蒸馏」加第二个人（Alex Hormozi，443 来源 / 2,726 条观点证据 / 1,596 个观点族）。加人这件事本身就是一次压力测试，`references/creator-craft.md` 按实战补了这些：

**新增能力**

- **§5.3 双轨建模的适用判据**。「方法论 + 人生观」双层在知识型博主里很常见，但不是每个人都该分轨；给了四条必须同时成立的判据，其中最硬的一条是**桥接找不出来就说明他只是换了话题，不是前提浮出**，这时应该退回单轨。
- **§5.2 第二人起的可选字段**：`tracks` / `bridges` / `models[].track` / `phases[].deepening` / `profile.seo` / `profile.disclosure` / `sampling_note`。全部可选，老数据不填照样过。
- **§5.4 利益披露做成固定模块**（不再是建议）。这个栏目里的人几乎都卖课，页面不写清楚就是在替人做免费漏斗。
- **§8 双轨与桥接的展示规格**，以及一条更要紧的提醒：「动态路由已存在」不等于「不用改页面」——正文里的统计胶囊、信源构成句、筛选器、免责声明人名、SEO 串全都是按第一个人写死的。

**门禁**

- 契约测试必须参数化（`import.meta.glob` + `describe.each`），数量断言改成「≥ 密度下限」而不是「=== 当前值」；新增真源断言「index 条目数 === 数据目录数」；**改完必须做变异测试确认会红**。
- 验收清单补四项：利益披露、抽样口径上页、分轨四判据、变异测试。

**踩坑（都是这一轮真撞出来的）**

- **两种 HTTP 400 长得一样、处理相反**：并发瞬时拒绝要拉长退避；内容审核拒绝是确定性的，重试多少次都一样，唯一的解是换通道。446 个来源里有 8 个撞上后者，换 Gemini 救回 7 条。
- **观点族归并只做字面相似度等于没做去重**：纯 Dice 最多合掉 22%，再调阈值先掉精度。必须「字面预合并 + 模型语义归并」两段，且**模型只许返回族 ID 分组、不许改写陈述**。
- **切批前必须按相似度把族排成链**：按 ID 切批会让跨批的重复观点永远不进同一个上下文，模型再准也合不掉。这个 bug 不报错、不缺字段，只体现为「合并率低得奇怪」。
- **引文核实加最长命中窗口回退**，实测核实率 94% → 98%。
- **抽样有系统性盲区**：只在书里讲透的动作会因为「视频里没人重复讲」而被族归并排到很后面，最终漏出总体蒸馏。要靠 P7-A 外部解读对照捞回来。

## [0.6.0] -- 2026-08-13

新增**可选的品牌浮标**：给蒸馏页左上角装一枚回站入口，让读者从这张单文件页面回得到你自己的站点。**默认关闭**，不配置就跟以前完全一样。

蒸馏页是设计成新标签页打开的单文件 HTML，天然脱离站点布局 -- 读者进来之后没有任何出口。把这样的页面挂上站点后才会发现：一整批页面全是死胡同，而每个页面单看都「正常渲染」，没有任何检查会报错。

**新增**

- 骨架新增可降级区块 `SLOT:BRANDBAR`：填 `brand` 配置才渲染，不填就连同注释整块删（与其余可降级区块同一套处理）。**未配置的用户不会继承任何品牌信息**，产出的仍是干净页面。
- 契约见 `references/html-spec.md §1.6`：`home_url`（母品牌首页）/ `series_url`（本系列列表页）/ `logo_light` / `logo_dark` / `series_label`。
- 配色全部走既有的 21 个主题 token，六套主题自动协调，不必为每套主题各写一遍；暗色主题靠两张 logo 显隐切换，比 `content:url()` 兼容。
- `verify_page.py` 新增 `lint_brandbar()`：区块不存在直接放行（这是默认且合法的状态）；一旦留下则逐条校验 -- 两个去向必须**互不相同**、logo 必须 `data:image` 内联、槽位必须填完。

**为什么强制两个去向不同**：logo 在所有人的认知里就是「回首页」。把它和系列名接到同一个地址，等于把一个人人都会做的动作接到了错误的地方 -- 读者点了 logo 却发现自己还在原地。这条是有真实事故的，所以做成门禁而不是建议。

**向后兼容**：已生成的页面无需重做；不配置 `brand` 时行为与上一版完全一致。

**验证**：全量 `python -m pytest` 为 **304 passed**，其中品牌浮标 8 条；每条门禁均做过变异测试，确认拿掉对应检查会失败。

## [0.5.1] -- 2026-07-30

### 修复

- 插件市场清单（`marketplace.json`）的版本号补回：0.5.0 发布时它漏跟，仍停在 0.4.1，从插件市场安装的人看到的是上一版的版本号。
- 变更记录的版本行统一为 `## [x.y.z] -- 日期`：0.5.0 那行误用单破折号，与本文件其余各版不一致。

## [0.5.0] -- 2026-07-30

新增 **StepC · 人物/博主蒸馏路径（creator_corpus）**：蒸馏对象从「一部作品」扩展到「一个人的跨媒介全部作品」（视频博主的全部视频 + 专栏/Newsletter + 书 + 播客）。

- `SKILL.md` 新增「先分流」表与 StepC 节，触发词补「蒸馏这个博主 / 人物思想蒸馏」；主管线（书 / 单视频 / 视频系列）行为不变。
- 新增 `references/creator-craft.md`：路由判定、「输入全量采集 / 分析完整建库 / 展示去重重构」总原则、五层数据模型（L0-L4）、P0-P9 阶段管线、来源卡批次门（模型只回 cue ID / 按 video_id 映射 / 一处系统性错误整批回退）、观点族跨媒介去重（同一观点只算一次）。
- 密度下限：核心观点 8-12 条分组归类、系统层 6-10、行动模型 8-12、时间线 4-6 段、内部张力 6-12；几百小时语料只蒸出几条口号判为收敛失败。
- 新增 P7 外部交叉核验（必做）：中英文第三方解读对照，做补缺 / 校准事实 / 找冲突三件事，每条带 URL。
- 新增 P7-B 通俗化两道闸：标题禁裸术语（人话标题 + 术语对照）、读者一分钟测试（60 秒答出他是谁 / 核心观点 / 为什么值得看 / 怎么演变 / 能学什么）。
- 展示层信息架构定型：核心思想 / 方法体系 / 思想轨迹 / 局限与争议 / 依据与来源五 tab + 作者简介子视图；数据流规则（上游为真源、导出器覆盖手改）。
- 首个验证案例：Dan Koe 人物蒸馏（162 视频 + 212 Letters + 25 书章 → 399 来源 / 1,836 证据 / 299 观点族），产物与数据契约见 `其他/Dan Koe/` 项目档案。
- 安装命令、插件元数据、姊妹 Skill 与版本链接已统一到当前公开仓地址，避免继续跳转到失效地址。

补三道**未填槽门禁**、一道**批量交付闸**,并新增轻量模型执行卡 -- 让 Gemini Flash 级模型 / 弱 agent 客户端也能按本 skill 的标准产出合格蒸馏页。

### 追加：公开出处脱敏门禁（2026-07-28）

格拉德威尔五册上线前复验发现，`distill.json` 为原文核验保留的 `第N章·book.txt:行号` 锚点，会被渲染器直接写入 `.src-note` 和 `data-source`，把内部输入文件名暴露给读者。根因不是原文锚点本身，而是没有分开“机器可追溯定位”和“读者可理解出处”两层表示。

- 公开渲染必须把内部锚点转换为“第N章·原文位置 N”形式；`distill.json` 原始锚点不改，仍供 `--source` 逐字核验和章节路由使用。
- `verify_page.py` 的 `lint_html()` 新增无条件 `book.txt` 扫描，任何公开 HTML（含属性、脚本或注释）残留内部文件名都会失败；`verify_batch.py` 自动继承。
- 补回归用例，确保“JSON 可保留 raw anchor、页面绝不泄露输入文件名”这条边界不会倒退；Step7 与 `html-spec.md` 同步写入交付规范。

**验证**：全量 `python -m pytest` 为 **296 passed**；格拉德威尔五页均通过原文、结构、富交互与截图验证，公开页 `book.txt` 命中为 0。

### 追加：原文事实门禁（2026-07-28）

一次格拉德威尔重蒸复核发现，旧页面即使通过结构检查，仍可能出现原文不存在的摘录、章节锚点错位，以及把同一段正文复制到多个章节凑字数。根因是 `verify_page.py` 只校验页面与 JSON 契约，没有把产物反查回用户提供的原文。

**新增**

- `verify_page.py --source <book.txt>`：与 `--distill` 联用时，逐条核验摘录去空白后的逐字命中、章节锚点是否指向该书真实章节，并拦截跨章节重复的 ≥120 字正文。
- 补齐同章注水与结构盲区：拦截 `narrative` 内重复的 ≥40 字句、读者可见的编辑流程语；顶层 `quotes` 也必须逐字命中原文；`尾声` 成为合法锚点。文章选编每篇须带并命中 `source_title`，从结构上阻止篇名—正文错配；文章选编新增 `render_profile.archetype="文章选编"`，采用 ≥300 字的独立篇目阈值，避免用论说书的 800 字硬门槛诱发凑字。
- Step7 的标准命令与交付条件已改为必须带 `--source`；没有原文就不能把页面标为完成。
- 三个回归用例覆盖：伪造摘录、虚构章节锚点、跨章重复正文。

**验证**：全量 `python -m pytest` 当前为 **295 passed**；其中新增回归覆盖文章选编的 300 字独立篇目阈值。

缘起是 2026-07-26 一次用 Antigravity(Gemini 3.6 Flash)蒸 6 本书的实测事故:5 分钟跑完、全部上站,事后核查发现 2 本只跑到 Step0 就停了(线上 404)、4 本 HTML 各残留 68 处 `{{槽}}` + 36 处 dummy、封面全是占位 SVG、distill.json 缺 8 个顶层键 —— **其中 1 本 `verify_page.py` 报 exit 0「全部通过」**。

根因不是模型写不出好内容(同批《异类》narrative 写到 824-1119 字,达标),而是**旧门禁 174 项全部是「遍历已有字段校验取值」**,默认「一定会填槽、一定会产全 schema」。模型把模板原样交付或少产半个 schema 时,循环空转 = 零违规放行。这三道补盲门与模型强弱无关,对所有模型同等生效。

**新增**
- **T0-P 未填槽门禁**(`lint_no_placeholder`,恒校验):成品页残留 `{{槽}}` 或 dummy 示例文案即 exit 1。`{{槽}}` 全文查;dummy **先剥 HTML 注释再查** —— 骨架顶部那条用法说明注释本身含「删 dummy/占位」字样、正常交付页普遍保留(全库 51 本里 47 本有,不渲染无危害),不剥会 100% 误报。校验 `templates/` 下的骨架模板时传 `lint_html(..., allow_placeholder=True)` 豁免。
- **T0-S schema 完整性门禁**(`lint_distill_schema`,恒校验):`method.md §6` 的 21 个顶层必需键缺失即 exit 1。空容器(`"quotes": []`)等同缺失;条件必需键按 `render_profile.omit_blocks` 豁免(语录/书单/课程/考试型);视频系列豁免 `concepts`/`credibility_verdict`。自造近义键名会被点名纠错(实测出现过把 `title` 写成 `book_title`)。**门禁在 `verify_pass1.py` 同样生效 —— 问题在 Pass1 就被拦下,不会一路带到 HTML。**
- **T0-C 真封面门禁**:占位 SVG(`data:image/svg+xml`)不再算合格封面。旧检查只查 `src^="data:image"`,占位 SVG 天然满足、形同虚设(全库正常蒸馏封面均为 jpeg/png/webp)。确实联网拿不到,须在 distill.json 顶层显式写 `"cover_fallback": true`,不许静默降级。
- **`scripts/verify_batch.py` 批量交付闸**(硬门禁④):上站前把**预期名单显式**交给它核对 —— 逐本核产物齐备 / `verify_page.py` 退出码 / 交付卫生(enrich 缺失、`_pass2_g*.json` 中间态残留),exit 0 才许上站。单本 verify 只回答「这一本合不合格」,回答不了「这一批该有的都在吗」;靠人肉数「应该都蒸完了吧」正是漏掉 2 本仍上站的病因。
- **`references/flash-mode.md` 轻量模型执行卡**:不降低任何质量标准,只把「靠自觉」的环节换成「可自检的判据」,逐步给完成判据与自检命令,并钉死六个最易滑落点(跳 Step7 / 槽没填完 / schema 少产 / 章数归并 / narrative 写到 300 字就收 / 少蒸的书仍进名单)。SKILL.md 顶部加指引。
- **Step0 目录识别门**(硬门禁①补充):`toc_detected: false` / `chapters_detected: 1` 也要停下问用户 —— 目录没识别出来 = 模型手里没有原书章节划分,章数只能自由发挥。
- **`convert_book.py` 支持套装/合集 epub 切分册**(`--list-volumes` / `--volume`):这正是上述「章数一律 6 章」的根因 —— 6 本全切自同一个「套装共5册」epub,而旧 `extract_epub` **完全不读 TOC**(把所有 ITEM_DOCUMENT 一股脑拼起来)、`diagnose` 又只用正文启发式正则猜章节。现改为:按 TOC 顶层定分册、按 **spine 区间**取正文(未列入目录的正文续页也收进来 —— 只取 TOC 列出的篇目会静默丢正文)、`title` 自动取分册名;`diagnose` 章数改**两路取大**(正文正则 vs TOC 声明)并新增 `chapters_source` 字段标明来源。真实套装 epub 实测:5 个分册全部正确识别与切分,标志词高频命中(盖蒂 29 / 薄片分析 33 / 肯纳 72)、跨书串扰零命中(一万小时 0 / 披头士 0)。

**测试**
- 新增 47 个单测(`scripts/tests/test_flash_gates.py` 37 + `test_convert_volume.py` 10):三道门禁的正反用例、omit_blocks / 视频豁免、别名纠错、注释豁免边界,以及一条复刻实测事故形态的回归断言。
- 全库回归:51 本已交付蒸馏页扫描,新门禁**零误报** —— 精确命中且仅命中 4 本问题产物。
- 测试基线 238 → **285 passed**。

**兼容性**
- `[schema]` 项对 2026-07-27 之前蒸的旧书会报 `render_profile`/`cover_intro` 等缺失,**属预期,旧书不必重蒸**(与既有 legacy 向后兼容策略一致)。T0-P / T0-C 两道对旧书零影响(实测 47 本全过)。

### 追加：富交互壳保真门禁（2026-07-28）

格拉德威尔重蒸复核发现，临时渲染器虽然能把 JSON 填进 HTML，也能通过旧版静态检查，却把正式骨架的主题切换、自绘脑图 viewer 与 hash 路由整段丢掉，形成“内容表面正确、阅读体验被降级”的隐蔽回归。

- verify_page.py 在传入 --distill 的单书页上新增富交互壳断言：必须保留 .theme-picker、initMindmap()、initHashRouter() 与脑图缩放控制；极简重写器直接 exit 1。
- Step6/Step7 明确：必须复制并逐槽填 page-skeleton.html，不能另造简壳来替代正式页面。
- 回归测试新增“数据页拒绝极简壳”；全量测试基线由 288 → 294 passed。

## [0.4.1] -- 2026-07-22

### 改进

- **触发边界更准确**：压缩自动触发描述，明确一本书全文、单集视频与视频系列均可进入深度蒸馏；只要字幕摘要、普通文章写作和「一页」专项不误触发。

## [0.4.0] -- 2026-07-15

新增 **StepB 主题聚合** -- 把同一主题下 ≥3 本已蒸作品横向重组成一张「分类地图 + 分歧矩阵 + 维度对照表 + 书目导航」的主题聚合页。这是继 StepA 作者演变(时间轴)之后的空间轴对称能力:StepA 看「一个作者怎么变」,StepB 看「一群书在同一议题上站哪边」。缘起是婴幼儿睡眠 13 本蒸馏 → 合成专题文的复盘 -- 手工把 13 本归成 7 派、挖出 3 条真分歧的活,值得沉淀成可复用的一步。

**新增**
- **StepB 管线 + `build_topic.py` 聚合器**:只读各书 distill.json + knowledge-index.json 做确定性集合运算(禁 LLM/联网),合并人工 `topic.manual.json`(流派归类/分歧分组/维度定义/怎么选)聚合成 topic.json。成员靠 `members:[slug]` 显式圈定;有效成员 <3 → exit 3 触发门槛;manual 缺失且 out 已存在 → 防覆盖栏 exit 2。
- **主题聚合页 4 视图**:分类地图(流派分区卡)/ 分歧矩阵(多列立场对照)/ 维度对照表(各书可执行数字并排、每格标 certainty 来源硬度)/ 书目导航。纯 HTML+CSS 表格卡片,不套 StepA 的坐标系 SVG,复用蒸馏页同源 token 与 Zero-Hex 铁律。
- **分歧诚实三档 `index_relation`**:`CONTRADICTS`(knowledge-index 已登记真对立,红旗)/ `curated`(编者从各书立场归纳、index 未登记,金标,须给依据)/ `parallel`(松散并列,聚合器剔除不渲)。把「index 往往只登记了少数 CONTRADICTS,但一个主题常有多条真分歧」这个现实如实呈现,而非硬造对立;编者归纳出 index 未登记的分歧应回补进 knowledge-index(顺带修 Step4 跨书分歧判定偏保守)。
- **每书页「主题入口卡」`SLOT:TOPIC-ENTRY`**:成员书蒸馏页顶部链到主题全景,<3 本或无 topic.json 生成时整卡删(可与作者演变入口卡并存)。
- **verify 独立门禁**:`is_topic_page` / `lint_topic_html` / `topic_smoke`(4 视图齐 / 零外链 / Zero-Hex / lang=zh / 破折号 / 深链格式 / 成员 ≥3 / index_relation + certainty 枚举 / 分歧可回指 / 渲染冒烟),照作者页那套对称加,走 main 短路分支绕开蒸馏页 REQUIRED_CLASSES。
- **契约 `references/topic-craft.md`**:topic.json schema / 4 视图数据契约 / 事实 vs 归纳分层铁律 / 成员圈定 / enrich 外部争议 / 入口卡,作为 build_topic + skeleton + verify 三者的单一权威。SKILL.md 管线表补 StepB 段(对称 StepA)。

**测试**
- 新增 26 个单测(15 topic 门禁 + 11 build_topic 聚合),覆盖 index_relation 三档判定、certainty 拉取/校验、门槛/防覆盖、破折号 quote 豁免、骨架静态门禁。clean-env pytest 全绿(238 passed)。

## [0.3.0] -- 2026-07-15

给「读者会照着做」的高后果书（育儿 / 医疗 / 理财…）加一层**数字可信度标注**，并把蒸馏产物的清理与验证边界写清楚。缘起是一次婴幼儿睡眠 13 本蒸馏 → 合成专题文的复盘：蒸馏本身很忠实，但下游引用时分不清哪些数字是「书里白纸黑字」、哪些是「编者补的通识」。

**新增**
- **后果轴 `stakes`（`high` / `normal`）**：与书型 / render_profile 正交的第三根轴，标记「读者照做、数字错会误导」的高后果书（育儿 / 医疗 / 用药 / 投资仓位 / 法律…）。缺省 `normal`，旧蒸馏无需回填。
- **可执行数字确定性 `certainty`**：`decision_rules[]` 与 `core_ideas[]` 元素级新字段，三值 `book_explicit`（书里白纸黑字）/ `cross_book_synthesis`（跨书合成）/ `general_knowledge`（编者通识），让下游一眼分清数字来源硬度；与 `evidence_level`（转述忠实度）正交共存。
- **门禁 G22**：`stakes=high` 时 `decision_rules` / `core_ideas` 每条必带合法 `certainty`（独立 stakes 闸，不随 render_profile；`normal` 书不检）。渲染侧复用 `.ci-evlevel` 色点，为决策卡加 `.rc-certainty` 来源硬度徽标。
- **G7 `evidence_level` 机检补齐**：`core_ideas` 必带 `evidence_level` 从「蒸馏自查」升为「verify 机检」。

**改进**
- **Pass2 中间态自动清理**：合并完整性门禁通过后删 `_pass2_g*.json`（已 gitignore、已回填 distill），治历史残留污染目录。
- **verify 边界澄清**：文档写清 `verify_page.py exit 0 ≠ 事实正确` -- 只保结构 / 契约 / 版权长度；内容真伪靠蒸馏自查 + 高后果书人工回原书抽检数字。

**修复**
- SKILL.md 硬门禁②的门禁编号从陈旧的 G1-G15 校正到 G1-G22。

## [0.2.0] -- 2026-07-12

这一版让蒸馏**按书型走不同形态**，并把批量蒸馏的成本压在编排上、而非砍内容深度。

**新增**
- **书型自适应 render_profile**：8 种书型原型（论说 / 叙事 / 人物 / 工具 / 语录 / 书单 / 课程 / 考试），按书型自动选输出形态与门禁档位 -- 语录书走语录墙、书单书走书单卡、课程 / 考试书走知识点树 + 考点卡，不再对所有书死搬同一套五段式。**向后兼容**：未标注书型的旧蒸馏页按原全门禁校验，无需重蒸。
- **4 类新原型页面原语**：语录墙 `.quote-board`、书单卡 `.booklist-cards`、知识点树 `.kp-tree`、考点卡 / 例题解析 / 记忆卡（`.exam-point` / `.worked-example` / `.recall-card`）。
- **门禁 Tier-0 / Tier-1 分层**：底线门禁（锚点 / 零外链 / 出处常显…）永不放宽；形态门禁（字数档 / 书魂 / 行动链…）随书型注册表变。verify 加书型完整性校验，防「自造书型 / 篡改 active_gates 偷绕门禁」。
- **批量编排效率规约**：跨会话并发闸（全局在飞 Pass2 subagent ≤ 6-8）、1 本书 = 1 会话防反复 compact、批前 token 预算、失败先核盘再定点 gap-fill（防假重跑）、Pass2 产物统一 `_pass2_gN.json` 命名、同作者 enrich 只搜一次。
- **Pass1 独立门禁** `verify_pass1.py`：骨架阶段即校验，UTF-8 控制台守卫修 GBK 假失败。
- **作者演变辅助脚本** `merge_enrich.py` / `merge_pass2.py`（数据根目录走 `DISTILL_DATA_DIR`）。

**修复**
- 兄弟 skill `sansheng-gemini-video` 链接改为完整 GitHub URL（独立扁平仓不再用 monorepo 相对路径）。
- 测试骨架路径按扁平仓结构修正（`parents[2] / templates`），clean-env pytest 全绿（208 passed）。

## [0.1.0] -- 2026-07-08

把一本书蒸成一张**能点、能跳、越读越厚**的交互网页 -- 脑图可点跳、章节可展开、7 主题切换、读完回看的自检问句，单文件 `file://` 直接打开。

**这一版包含：**
- 八步蒸馏管线：入书诊断 → 书型判定 → 四源融合蒸馏 → 联网增补 → 跨书索引 → 设计两遍工作法 → 单文件 HTML → 出厂验证
- 两类输入：书籍电子全文（epub / pdf / txt / azw3 / mobi），以及视频系列（每集当章节）
- 跨书概念索引：越蒸越厚的个人知识网络，新书自动与旧书互链
- 产出为零外链单文件 HTML，可离线 `file://` 打开

装法与网页演示见 README。这是叁笙自己每天在用、清洗脱敏后开源的 Claude Code 技能。

[0.7.0]: https://github.com/sanshengai/sansheng-distill/tree/v0.7.0
[0.6.0]: https://github.com/sanshengai/sansheng-distill/tree/v0.6.0
[0.5.1]: https://github.com/sanshengai/sansheng-distill/tree/v0.5.1
[0.5.0]: https://github.com/sanshengai/sansheng-distill/tree/v0.5.0
[0.4.1]: https://github.com/sanshengai/sansheng-distill/tree/v0.4.1
[0.4.0]: https://github.com/sanshengai/sansheng-distill/tree/v0.4.0
[0.3.0]: https://github.com/sanshengai/sansheng-distill/tree/v0.3.0
[0.2.0]: https://github.com/sanshengai/sansheng-distill/tree/v0.2.0
[0.1.0]: https://github.com/sanshengai/sansheng-distill/tree/v0.1.0
